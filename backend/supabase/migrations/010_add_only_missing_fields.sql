-- Migration: Add only missing political party fields
-- This migration checks what exists and only adds what's missing

-- First, let's see what columns already exist
-- You can run this query in Supabase to see what's already there:
-- SELECT column_name FROM information_schema.columns WHERE table_name = 'congress_initiatives';
...
-- Create the junction table for party relationships (only if it doesn't exist)
